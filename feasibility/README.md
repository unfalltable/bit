...
| 真实四节点 + Rust ABCI/JMT | Transfer、ClaimGenesis、Unbond/ClaimExit、供应守恒、同高度状态一致、H+2、应用重启、真实重复投票处罚、ICS23、投票权不足停止及恢复检查通过 |
| 手机 | 用户跳过；ADB 服务已停止 |

密码学程序已经组成并验收完整 BIT Transfer；早期 proof probe 只写入内存账本夹具，当前四节点网络已复用生产 `bit-app` 与 JMT/RocksDB 状态核心执行真实 Transfer、ClaimGenesis、Unbond 和 ClaimExit。它还通过标准 CometBFT RPC 注入真实重复投票证据并验证处罚，但没有独立 signer。四个节点运行于同一宿主。总体状态仍是部分开发验证通过。

## 目录

- `proof-probe/`：真实 Penumbra v2.1.1 原语和扫描实验，包含 Cargo.lock。
- `proof-probe/src/bin/bit-exit-fixture.rs`：为隔离四节点网络动态生成并自验真实 Unbond/ClaimExit 信封。
