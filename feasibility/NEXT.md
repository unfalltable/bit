...
后续正式开发已经完成持久化 commitment tree、nullifier/anchor 索引、原子提交、ICS23 状态证明，并把 Transfer、九类质押信封、退出和罚没资金闭环接入 ABCI/JMT。最新四节点实验还通过标准 CometBFT RPC 注入真实重复投票证据，验证规范证据记录、Burn、H+2 移除、同高度 app hash 和处罚后的投票权停止/恢复。状态 Commit 已启用同步 WAL、错误传播和 WriteBatch 预写校验，并覆盖处罚提交两侧的确定性故障恢复。状态层也已实现绑定完整状态摘要、4 MiB chunk hash、严格文件集和配置校验的本地 checkpoint 快照，以及只在隔离打开验证通过后发布的恢复流程。当前下一阶段是真实磁盘耗尽/fsync 失败、ABCI State Sync/轻客户端可信根、历史证明、生产摘要与正式节点命令；独立 signer 仍未实现。
