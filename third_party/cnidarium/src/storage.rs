...
#[cfg(feature = "bit-fault-injection")]
...
const WRITE_BATCH_HEADER_BYTES: usize = 12;
const TYPE_COLUMN_FAMILY_DELETION: u8 = 0x4;
const TYPE_COLUMN_FAMILY_VALUE: u8 = 0x5;

// Cnidarium 0.83 only emits column-family puts and deletes. Validate the
// complete RocksDB wire batch before it can enter the WAL; RocksDB itself may
// accept a truncated batch and only report corruption on the next open.
fn validate_write_batch(batch: &rocksdb::WriteBatch) -> Result<()> {
    let bytes = batch.data();
    ensure!(
        bytes.len() >= WRITE_BATCH_HEADER_BYTES,
        "RocksDB WriteBatch is shorter than its header"
    );
    let expected_records = u32::from_le_bytes(
        bytes[8..12]
            .try_into()
            .expect("the WriteBatch header length was checked"),
    );
    let mut offset = WRITE_BATCH_HEADER_BYTES;
    for _ in 0..expected_records {
        let tag = *bytes
            .get(offset)
            .ok_or_else(|| anyhow::anyhow!("RocksDB WriteBatch record tag is truncated"))?;
        offset += 1;
        match tag {
            TYPE_COLUMN_FAMILY_DELETION => {
                take_batch_varint32(bytes, &mut offset)?;
                take_batch_varstring(bytes, &mut offset)?;
            }
            ...
            _ => bail!("unsupported or corrupt RocksDB WriteBatch tag {tag:#04x}"),
        }
    }
    ensure!(
        offset == bytes.len(),
        "RocksDB WriteBatch record count or trailing bytes are invalid"
    );
    Ok(())
}

fn take_batch_varint32(bytes: &[u8], offset: &mut usize) -> Result<u32> {
    let mut value = 0u32;
    for index in 0..5 {
        let byte = *bytes
            .get(*offset)
            .ok_or_else(|| anyhow::anyhow!("RocksDB WriteBatch varint is truncated"))?;
        *offset += 1;
        if index == 4 && byte > 0x0f {
            bail!("RocksDB WriteBatch varint overflows u32");
        }
    ...
    bail!("RocksDB WriteBatch varint is unterminated")
}

fn take_batch_varstring<'a>(bytes: &'a [u8], offset: &mut usize) -> Result<&'a [u8]> {
    let length = usize::try_from(take_batch_varint32(bytes, offset)?)
        .map_err(|_| anyhow::anyhow!("RocksDB WriteBatch value length does not fit usize"))?;
    let end = offset
        .checked_add(length)
        .ok_or_else(|| anyhow::anyhow!("RocksDB WriteBatch value length overflows usize"))?;
    let value = bytes
        .get(*offset..end)
        .ok_or_else(|| anyhow::anyhow!("RocksDB WriteBatch value is truncated"))?;
    *offset = end;
    Ok(value)
}
...
/// One-shot commit failures exposed only to BIT's deterministic fault tests.
#[cfg(feature = "bit-fault-injection")]
...
pub enum CommitFault {
    BeforeWrite = 1,
    CorruptWriteBatch = 2,
    AfterWriteBeforeCachePublication = 3,
}
...
impl Storage {
    ...
    /// Arm one deterministic, process-local commit failure for integration tests.
    #[cfg(feature = "bit-fault-injection")]
    pub fn inject_commit_fault_once(&self, fault: CommitFault) {
        self.0.commit_fault.store(fault as u8, Ordering::SeqCst);
    }

    #[cfg(feature = "bit-fault-injection")]
    fn take_commit_fault(&self, fault: CommitFault) -> bool {
        self.0
            .commit_fault
    ...
    pub fn commit_batch(&self, batch: StagedWriteBatch) -> Result<crate::RootHash> {
        #[cfg(feature = "bit-fault-injection")]
        if self.take_commit_fault(CommitFault::BeforeWrite) {
            bail!("injected RocksDB failure before atomic batch write");
        }
        ...
        #[cfg(feature = "bit-fault-injection")]
        if self.take_commit_fault(CommitFault::CorruptWriteBatch) {
            let mut bytes = write_batch.data().to_vec();
            ensure!(
                !bytes.is_empty(),
                "cannot inject corruption into an empty RocksDB WriteBatch"
            );
            bytes.pop();
            write_batch = rocksdb::WriteBatch::from_data(&bytes);
        }

        validate_write_batch(&write_batch)?;

        // ABCI Commit must not return until the WAL entry is durable. Keep the
        // WAL enabled and request an fsync for the complete atomic WriteBatch.
        let mut write_options = WriteOptions::default();
        write_options.set_sync(true);
        db.write_opt(write_batch, &write_options)?;

        #[cfg(feature = "bit-fault-injection")]
        if self.take_commit_fault(CommitFault::AfterWriteBeforeCachePublication) {
            bail!("injected process failure after durable write before cache publication");
        }
