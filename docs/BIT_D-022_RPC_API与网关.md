...
状态证明不是任意数据库反射。键最长 256 ASCII 字节；只允许固定网络/费用/TCT 根/供应键，以及 transaction、nullifier、区块摘要和选定公开 staking 记录前缀。TCT frontier、子存储版本标记、内部推进队列、SlashJob 游标和未知键空间不会由网关暴露。缺失、无法解析或越界的查询参数统一返回 JSON `E_BAD_QUERY` 或对应的稳定错误码。
