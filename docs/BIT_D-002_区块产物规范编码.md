...
状态：`IN_PROGRESS`。版本 1 的执行摘要与紧凑区块字节合同、严格解码器、域分离哈希、共享黄金向量和 ABCI/状态接线已经实现。产物归档、按高度下载接口、保留策略和轻客户端消费仍待 D-003/D-010 完成。
...
当前 ABCI 事件只公开摘要与紧凑区块长度。完整产物的不可变归档、服务接口、历史证明与客户端扫描属于下一阶段，不能仅凭本实现宣称恢复服务已经完成。
