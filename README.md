...
- `crates/bit-transaction`：完整 Transfer 以及 RegisterValidator、Delegate、CancelPending、UpdateValidator、UnjailValidator、RotateConsensusKey envelope 的链上下文、费用、Spend/Output 证明、Spend 授权、角色域分离 Ed25519 签名、共识键 PoP、公开价值 binding 和重复 nullifier 检查。
- `crates/bit-emission`：固定 1024 亿枚上限的整数会计，验证已发行量、当前供应量、容器总额、销毁量、已放弃额度和未来发行额度之间的恒等式；同时实现无追补的逐 epoch 发行规则和按规范交易大小、证明数量、动作类型计算的最低费。
- `crates/bit-staking`：验证者、质押池与持仓的确定性账本；实现 pending 容量、下一 epoch 激活、U256 份额换算、滑点退款、自质押资格、候选排序、投票权安全边界、验证人序号、佣金延迟、双门槛解禁和共识密钥轮换历史。
- `crates/bit-state`：基于 Cnidarium 0.83/JMT 0.11/RocksDB 8.1.1 的 schema v8 状态，维护真实 Penumbra TCT、创世承诺、链时间、anchor 窗口、nullifier、交易索引、供应会计、费率配置及逐项质押记录，并提供两阶段原子提交和 ICS23 查询证明；真实 Transfer 和六类质押交易均已通过落盘、证明、重启和篡改拒绝测试。
...
当前还没有生产 execution/compact 摘要编码、真实 CometBFT 多节点接入、基于验证人签名得分的 epoch 奖励分配、ValidatorUpdates、在线率窗口、退出/罚没动作、独立 signer、钱包或主网发布能力。epoch 高度与结算计数已经强制一致，系统入口要求在 epoch 首块先结算发行再激活质押；奖励计划缺失时仍不能声称完整结算。`supply/audit_snapshot` 的规范编码仍等待 SPEC-04 决议。持久化层仍需补故障注入、快照导入导出、历史证明和长期增长测试。`config/mainnet-inputs.template.json` 保持阻断状态。
