...
- `crates/bit-app`：确定性生命周期核心及固定 CometBFT ABCI v0.38 适配，严格解析实际 last commit、`next_validators_hash` 和 `FinalizeBlock.misbehavior`，按固定系统顺序完成计分、证据处罚、发行奖励、pending 激活、集合选择与有界队列推进，返回验证人 key/power 更新及自验后的 ICS23 ProofOps；State Sync 接受 CometBFT 轻客户端提供的可信 app hash，逐块校验后在隔离数据库恢复并以耐久标记切换活动状态。
