...
- `crates/bit-state`：基于 Cnidarium 0.83/JMT 0.11/RocksDB 8.1.1 的 schema v14 状态，维护真实 Penumbra TCT、创世承诺、链时间、anchor 窗口、nullifier、交易索引、供应会计、逐项质押/退出记录、持久化退出推进队列、增量候选索引及 H/H+1/H+2 实际验证者集合，并提供两阶段原子提交和 ICS23 查询证明；真实 Transfer 和九类质押交易均已通过正式信封入口，其中退出链路另覆盖跨块成熟、落盘证明和重启恢复。
