...
- `crates/bit-gateway`：基于同一 `ApplicationCore` 的只读 Axum 网关，首批提供健康检查、白名单状态证明、规范供应审计和有界 compact block 下载。所有 compact 字节都与同高历史证明交叉核对；未配置 TLS 前只允许 loopback 监听。
