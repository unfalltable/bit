...
活跃工作区已包含规范类型与交易编码、真实 Groth16/RedDSA/Ed25519 验证、持久状态与精确历史证明、固定总量发行、原生质押/退出/处罚、ABCI、正式创世节点、四节点 CometBFT 场景、State Sync、只读网关以及签名检查点可信期内核。它仍不是完整钱包或主网上线交付；移动端、完整轻客户端、运维部署、独立审查和真实主网外部材料仍需完成。
...
`SPEC-05` 的检查点 v1 合同已经冻结：策略由创世审批阈值授权并绑定 genesis、chain context 与运行时参数；检查点使用独立发布者阈值，显式包含 H/H-1、头/app hash、生成与到期时间；轮换采用连续序号、前一策略哈希和立即撤销位。可信期必须短于解除质押期，过期或冲突后只能用独立核对的精确 checkpoint hash 显式重建信任。钱包持久化与完整头验证属于 D-010 后续实现，不重新打开该字节合同。
